...
public class GlowEffect : MonoBehaviour
{
    ...
    Material m_ComposInventoryItematerial = null;
	protected Material composInventoryItematerial {
		get {
			if (m_ComposInventoryItematerial == null) {
                m_ComposInventoryItematerial = new Material(compositeShader);
				m_ComposInventoryItematerial.hideFlags = HideFlags.HideAndDontSave;
			}
			return m_ComposInventoryItematerial;
		} 
	}
	...
	{
		if( m_ComposInventoryItematerial ) {
			DestroyImmediate( m_ComposInventoryItematerial );
		}
		if( m_BlurMaterial ) {
	...
	protected void Start()
	{
		...
		{		
			if( !blurMaterial.shader.isSupported )
				enabled = false;
			if( !composInventoryItematerial.shader.isSupported )
				enabled = false;
	...
	{
		composInventoryItematerial.color = new Color(1F, 1F, 1F, Mathf.Clamp01(glowIntensity));
		Graphics.Blit (source, dest, composInventoryItematerial);
	}	
}
